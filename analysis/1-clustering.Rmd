...
```{r load-data}
sce <- readRDS(file.path("output", "sce_prep.rds"))
source(file.path("code", "utils.R"))
```

# Clustering

```{r Seurat, message = FALSE, fig.width = 5, fig.height = 3}
# create `SeuratObject`
so <- CreateSeuratObject(
    counts = assay(sce, "counts"), project = "LPM",
...
```{r clustree, fig.width = 10, fig.asp = 1}
...
```{r dp1, fig.width = 10, fig.asp = 1/2}
...
```{r hm1, fig.width = 12, fig.asp = 2/3}
DoHeatmap(so, gs)
```
...
gs <- group_by(mgs, cluster) %>% 
    dplyr::slice(seq_len(4)) %>% 
    pull("gene") %>% unique
```

### Dot plot

```{r dp2, fig.width = 16, fig.asp = 2/3}
DotPlot(so, features = gs) + 
    RotatedAxis() + coord_cartesian(clip = "off") +
    theme(plot.margin = unit(c(0,0,0,4), "cm")) +
    labs(x = NULL, y = NULL)
...
```{r hm2, warning = FALSE, fig.width = 12, fig.asp = 2/3}
DoHeatmap(so, gs)
```
...
# write out files
saveRDS(so, file.path("output", "so_clust.rds"))
saveRDS(sce, file.path("output", "sce_clust.rds"))
write.csv(mgs, file.path("output", "mgs_Seurat.csv"))
```
